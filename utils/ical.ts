...
const intervalGraphColoring = (events: Omit<Event, 'priority' | 'max_priority'>[]): Event[] => {
  const sortedEntries = events.flatMap(event => [
  ...
  for (const entry of entries) {
    if (entry.type === 'start') {
      ...
      stack.forEach(entry => { entry.max_priority = Math.max(entry.max_priority, maxPriority) })
      stack.pop()
    }
  ...
  return entries
    ...
    .map(entry => {
      const event: Omit<Event, 'priority' | 'max_priority'> = events.find(event => event.uuid === entry.uuid) as Omit<Event, 'priority' | 'max_priority'>
      return { ...event, priority: entry.priority, max_priority: entry.max_priority }
    })
}

const serialize = (events: Event[]): SerializedEvent[] => events.map(event => ({
  ...event,
  start: event.start.toISOString(),
  end: event.end.toISOString()
}))

const deserialize = (events: SerializedEvent[]): Event[] => events.map(event => ({
  ...event,
...
const load = (url: string): Promise<SerializedEvent[]> => fetch(url)
  ...
  .then(text => {
    ...
    const events: (Omit<Event, 'priority' | 'max_priority'> & { priority: null, max_priority: null })[] = icalEvents.map(event => ({
